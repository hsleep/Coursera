...
object Huffman {
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeInner(subTree: CodeTree, subBits: List[Bit], agg: List[Char]): List[Char] = {
      if (subBits.isEmpty) agg
      else {
        val Fork(left, right, _, _) = subTree
        val selected = subBits.head match {
          case 0 => left
          case 1 => right
        }
        selected match {
          case _: Leaf => decodeInner(tree, subBits.tail, agg ++ chars(selected))
          case _: Fork => decodeInner(selected, subBits.tail, agg)
        }
      }
    }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertInner(subTree: CodeTree, agg: List[Bit] = Nil): CodeTable = {
      subTree match {
        case Leaf(char, _) => List(char -> agg)
        case Fork(left, right, _, _) =>
          mergeCodeTables(convertInner(left, agg :+ 0), convertInner(right, agg :+ 1))
      }
    }
    convertInner(tree)
  }
