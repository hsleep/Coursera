...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.groupBy(identity).map { case (c, s) =>
      c -> s.length
    }.toList.sorted
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def comb(c: Char, n: Int, agg: List[Occurrences]): List[Occurrences] = {
      for {
        o <- agg
        i <- 0 to n
      } yield {
    ...
    def inner(occurrences: Occurrences, agg: List[Occurrences]): List[Occurrences] = {
      occurrences match {
        case head::tail =>
          val (c, n) = head
          inner(tail, comb(c, n, agg))
        case Nil => agg
      }
    }
    inner(occurrences, List(Nil))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val mapY = y.toMap
    x.foldLeft(List.empty[(Char, Int)]) { case (agg, (c, n)) =>
      n - mapY.getOrElse(c, 0) match {
        case 0 => agg
        case i => agg :+ c -> i
      }
    }
